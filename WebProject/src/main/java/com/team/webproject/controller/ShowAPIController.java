	 ...
	 //@RequestMapping(value="/api", produces = MediaType.APPLICATION_JSON_VALUE)
	    public String callAPI(Model model) throws JsonProcessingException {
	        ...
	        try {
	            ...
	            String url = "http://www.kopis.or.kr/openApi/restful/pblprfr/PF222348";
	            ...
	            System.out.println(jsonob.get("mt20id"));
	            model.addAttribute("code", jsonob.get("mt20id"));
	            System.out.println(jsonob.get("prfnm"));
	            model.addAttribute("name", jsonob.get("prfnm"));
	            System.out.println(jsonob.get("prfpdfrom"));
	            model.addAttribute("stdate", jsonob.get("prfpdfrom"));
	            System.out.println(jsonob.get("prfpdto"));
	            model.addAttribute("endate", jsonob.get("prfpdto"));
	            System.out.println(jsonob.get("poster"));
	            model.addAttribute("post", jsonob.get("poster"));
	            System.out.println(jsonob.get("genrenm"));
	            model.addAttribute("category", jsonob.get("genrenm"));
	            System.out.println(jsonob.get("fcltynm"));
	            model.addAttribute("place", jsonob.get("fcltynm"));
	            System.out.println(jsonob.get("styurls"));
	            
	            String test = jsonob.get("styurls").toString();
	            JSONObject test2 = (JSONObject) parser.parse(test);
	            
//	            model.addAttribute("guidepost", jsonob.get("fcltynm"));
//	            JSONObject jsonstyuri = (JSONObject) parser.parse((String) jsonob.get("styurl"));
	            
	            model.addAttribute("guidepost", test2.get("styurl"));
