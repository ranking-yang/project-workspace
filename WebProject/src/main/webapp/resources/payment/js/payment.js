 ...
 $(document).ready(function() {
    ...
    $('#payment-btn').click(function() {
      ...
      if ($('#payment-chk').prop('checked')) {
		...
		if (parseInt($('#payment-total-Price').text()) === 0) {
			// 무료로 구매일 때
...
function forFree() {
	makePaymentDTO_forFree(createOrderNum()); // 결제 DTO
	$("#payment_success_form").submit(); // 서브밋	
}
...
function makePaymentDTO_forFree(code) {
	// 결제 완료 후, 결제 DTO 생성
	let payment_code = $('<input>', {
        type: 'hidden',
        name: 'payment_code',
    ...
    let total_price = $('<input>', {
        type: 'hidden',
        name: 'total_price',
        value: parseInt($('#payment-total-Price').text())
    });
...
$('#reset').click(function() {
	...
	$('#coupon-value').text('');
	$('#member-coupon-code').val('');
	const basePrices = $('.base-price');
	let totalPrice = 0;
